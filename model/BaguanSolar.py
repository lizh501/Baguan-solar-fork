...
def interpolate_btchw(x, size=None, scale_factor=None, mode="bilinear", align_corners=False):
    """
    x: Tensor [B, T, C, H, W]
    size: (H2, W2) or None
    scale_factor: float or (sh, sw) or None
    """
    B, T, C, H, W = x.shape
    x2 = x.reshape(B * T, C, H, W)

    y2 = F.interpolate(
        x2, size=size, scale_factor=scale_factor,
        mode=mode, align_corners=align_corners if mode in ("linear","bilinear","bicubic","trilinear") else None
    )

    H2, W2 = y2.shape[-2:]
    y = y2.reshape(B, T, C, H2, W2)
    return y
...
class CrossAttnBlock(nn.Module):
    def __init__(self, embed_dim, num_heads, attn_drop_rate=0.0, drop_path_rate=0.0):
        super().__init__()
        self.cross_attn = nn.MultiheadAttention(
            embed_dim=embed_dim, num_heads=num_heads,
            dropout=attn_drop_rate, batch_first=True
        )
        self.norm = nn.LayerNorm(embed_dim)
        self.drop = nn.Dropout(drop_path_rate)

    def forward(self, q_feat, kv_feat):
        # q_feat: [B, Lq, C], kv_feat: [B, Lk, C]
        attn_out, attn_w = self.cross_attn(
            query=q_feat, key=kv_feat, value=kv_feat, need_weights=False
        )
        # 残差 + norm（常见Transformer风格）
        out = self.norm(q_feat + self.drop(attn_out))
        return out

class BaguanSolar(nn.Module):
    def __init__(self, params_cfg, target_size, **kwargs):
        super().__init__()
        self.target_size = target_size
        self.sate_encoder = MultiDecoderSwinNet(**params_cfg["SateEncoderSwinNet"])
        self.env_encoder = MultiDecoderSwinNet(**params_cfg["EnvEncoderSwinNet"])
        self.cross_sat = CrossAttnBlock(**params_cfg["SateCrossAttnBlock"])
        self.decoder_1 = MultiDecoderSwinNet(**params_cfg["MultiDecoderSwinNet_Stage1"])
        self.decoder_2 = MultiDecoderSwinNet(**params_cfg["MultiDecoderSwinNet_Stage2"])

    def forward(self, satellite, env_data, clearghi): 
        # satellite: [-1, 1],   clearghi: [0, 1]
        # encoder
        satellite = interpolate_btchw(satellite.permute(0, 2, 1, 3, 4), size=(self.target_size, self.target_size), mode="bilinear", align_corners=False)
        env_data = interpolate_btchw(env_data.permute(0, 2, 1, 3, 4), size=(self.target_size, self.target_size), mode="bilinear", align_corners=False)
        clearghi = interpolate_btchw(clearghi.unsqueeze(2), size=(self.target_size, self.target_size), mode="bilinear", align_corners=False)

        B, T, C, H, W = satellite.shape
        satellite = rearrange(satellite, 'b t c h w -> b (c t) h w')
        env_data1 = env_data[:, :, :39]
        env_data1 = rearrange(env_data1, 'b t c h w -> b (c t) h w')
        B, D, H, W = satellite.shape

        sate_feat = self.sate_encoder(satellite)   # [B, H//p * W//p, embed_dim]
        env_feat1 = self.env_encoder(env_data1)      # [B, H//p * W//p, embed_dim]

        cloud_and_sate = self.decoder_1(torch.cat([self.cross_sat(sate_feat, env_feat1), env_feat1], dim=2))
        cloud_and_sate = rearrange(cloud_and_sate, 'b (c t2) h w -> b c t2 h w', c=C+1)
        cloud_pred_norm = cloud_and_sate[:, 0:1, :, :, :].permute(0, 2, 1, 3, 4).contiguous()               # [B, T, 1, H, W] in [-1, 1]
        sat4_pred = cloud_and_sate[:, 1:, :, :, :].permute(0, 2, 1, 3, 4).contiguous()    
        
        B, T, _, H, W = cloud_pred_norm.shape                  # [B, T, 1, H, W]

        cloud_pred = (100 - 0) / 2 * (cloud_pred_norm + 1) + 0   # [0， 100]
        # cloud = cloud.squeeze(1)    # [B, T, H, W]
        
        # crop and padding
        lead_time = torch.arange(1, T+1, device=cloud_pred.device).view(1, T, 1, 1, 1)
        lead_time = lead_time.expand(B, T, 1, H, W) / 24

        ghi_list = []
        # env_data: (B, 41, 30, H, W) = his(6) + fut(24)
        fut_era5 = env_data[:, -T:, :, :, :]  # (B, 41, 24, H, W)

        era5_stage2_idx = [7, 8, 9, 10, 11, 12, 13, 37, 38, 39, 40]  # q + shortwave
        fut_era5_sel = fut_era5[:, :, era5_stage2_idx, :, :]  # (B, 24, 9, H, W)
        
        for t in range(T):
            x_t = torch.cat([cloud_pred_norm[:, t], clearghi[:, t], lead_time[:, t], sat4_pred[: , t], fut_era5_sel[:, t]], dim=1)  # (B, 7+11=18, H, W)

            ghi_t = self.decoder_2(x_t)   # 输出 [B,H,W]
            ghi_t[clearghi[:, t:t+1, 0] == 0] = 0 # mask 黑天为0
            ghi_list.append(ghi_t)
        ghi_pred = torch.cat(ghi_list, dim=1).unsqueeze(2)   # [B,T, 1,H,W]

        ghi_pred = interpolate_btchw(ghi_pred, size=(512, 512), mode="bilinear", align_corners=False)
        cloud_pred = interpolate_btchw(cloud_pred, size=(512, 512), mode="bilinear", align_corners=False)
        sat4_pred = interpolate_btchw(sat4_pred, size=(512, 512), mode="bilinear", align_corners=False)

        return dict({
            "pred_ghi": ghi_pred.squeeze(2) * 1500.,     # [0, 1500]
            "pred_cloud": cloud_pred.squeeze(2),  # [0， 100]
            "pred_satellite": sat4_pred
        })